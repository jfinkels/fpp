...
\end{lemma}
\begin{proof}
  The reduction operates as follows on input circuit $C$ of size $m$ and depth $t$.
  Construct a trie (also known as a prefix tree) from the $O(m^t)$ possible paths from output gate to input gate, then output the Boolean formula $\phi$ represented by the constructed trie.
