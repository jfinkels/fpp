...
\subsection{Defining classes with built-in complete problems}
...
\subsection{Constructing complete problems from existing classes}

This theorem is inspired by \autocite[Corollary~2]{cd98}, which proves a related problem is complete for the class $\PNC$ (a class that lives between $\FPP$ and $\FPT$) under $\FPP$ many-one reductions.

\begin{definition}[$\pbsbdce$]
  \mbox{} \\
  \begin{tabular}{r p{9.2cm}}
    \textbf{Instance:} & Boolean circuit $C$ on $n$ inputs, binary string $x$, natural numbers $\alpha$ and $\beta$, circuit-computable functions $f$ and $g$, positive integer $k$, where $\size(C) \leq f(k) n^\alpha$ and $\depth(C) \leq g(k) n^\beta$. \\
    \textbf{Parameter:} & $k$. \\
    \textbf{Question:} & Does $C(x) = 1$?
  \end{tabular}
\end{definition}
...
\begin{theorem}
  $\pbsbdce$ is complete for $\FPT$ under $\FPP$ many-one reductions.
\end{theorem}
\begin{proof}
  \todo{I tested this out on paper already, it works; just need to write it up.}
\end{proof}

As a corollary, if this problem is fixed-parameter parallelizable, then $\FPP = \FPT$.
\todo{Show negative consequences of $\FPP = \FPT$; \autocite{cd98} already basically did this, but not clearly.}
