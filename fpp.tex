...
\begin{proof}
  ...
  We will use \autoref{lem:reducetonc} to show a reduction meeting the criteria from an arbitrary decision problem $Q$ in $\NNC^d[i(n) \log n]$ to $\pncsat$.
  \todo{Could we use a complete problem here instead of an arbitrary problem?}

  Let $\{C_{m, k'}\}$ be the deterministic circuit family, and $f$ and $g$ the circuit-computable functions such that
  \begin{itemize}
...
\todo{Move this up to a section on characterizations of FPP}
...
\begin{theorem}
  Suppose $d$ and $t$ are positive integers with $t \geq 2$.
  If $\FPP^d = \WP^d[t]$, then there is a circuit-computable, unbounded, nondecreasing function $i$ such that %% $i(n) \leq n$ and
  $\GC[i(n) \log n, \Pi_t \LOGTIME]$.
\end{theorem}
\begin{proof}
  Assume $\FPP^d = \WP^d[t]$.
  Since ... is in $\WP^d[t]$, it is now in $\FPP^d$ as well.
