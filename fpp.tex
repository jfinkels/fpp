...
\begin{definition}[$p \dash \textsc{Weighted Satisfiability}$ (aka $\pFSAT$)]
  \mbox{} \\
  \begin{tabular}{r p{9.2cm}}
    \textbf{Instance:} & Boolean formula $\phi$, natural number $k$. \\
    \textbf{Parameter:} & $k$. \\
    \textbf{Question:} & Is there a binary string $x$ such that $\|x\|_1 = k$ and $\phi(x) = 1$?
  \end{tabular}
\end{definition}

\begin{theorem}[{\autocite[Theorem~3.6]{est15}}]\label{thm:parawnc1complete}
  $\pFSAT$ is complete for $\para \WNC^1$ under $\para \FO$ many-one reductions.
\end{theorem}
...
There the authors prove that as a decision problem, the weighted $\NC^d$ circuit satisfiability problem is complete for $\NNC[k \log n]$.
...
\begin{theorem}\label{thm:parawncdcomplete}
  For each positive integer $d$ greater than one, we have $\pNCSAT$ is complete for $\para \WNC^d$ under $\para \NC^1$ many-one reductions.
\end{theorem}

The proof of this theorem requires an auxiliary function, $\psi$, that decodes a binary string of Hamming weight exactly one into the index, as an integer in binary, of the one in that string.
Define $\psi \colon \{0, 1\}^n \to \{0, 1\}^{\log n}$ as
\[
\psi(x) =
\begin{cases}
  i & \text{if } x = 0^{i - 1}10^{n - i} \\
  0^{\log n}& \text{otherwise},
\end{cases}
\]
for each binary string $x$ of length $n$.
The function $\psi$ is surjective: for each binary string $i$ of length $\log n$ there is an $x$ such that $\psi(x) = i$.
Another way of understanding $\psi$ is by its inverse: $\psi^{-1}$ encodes each nonzero binary string of length $\log n$ as a binary string of length $n$ of Hamming weight exactly one.
This will be useful for enforcing an upper bound on the Hamming weight of satisfying inputs to the circuit constructed in the reduction.

\begin{lemma}
  $\psi$ is circuit-computable in size ... and depth ...
\end{lemma}
\begin{proof}
  \todo{fill me in}
\end{proof}

\begin{proof}[Proof of \autoref{thm:parawncdcomplete}]
  \todo{Show membership.}
  ...
  Let $\Psi$ denote the function $\Psi(w) = \psi(w_1) \circ \dotsc \circ \psi(w_{h(k)})$, where $w_i$ is the $i$th block of size $n$ in the string $w$, for each string $w$ of length $h(k) n$.
  The reduction is then $x \mapsto (D, h(k))$, where $D = C_x \circ \Psi$.

  The circuit $D$ is of the correct form to be an input to $\pNCSAT$.
  The size of $D$ is
  \begin{align*}
    \size(D) & = \size(\Psi) + \size(C_x) \\
    & \leq h(k) \size(\psi) + f(k) n^{O(1)} \\
    & \leq h(k) n + f(k) n^{O(1)}
  \end{align*}
  and the depth
  \begin{align*}
    \depth(D) & = \depth(\Psi) + \depth(C_x) \\
    & \leq \depth(\psi) + (f(k) + \log^d n) \\
    & \leq \log n + f(k) + \log^d n \\
    & \leq f(k) + 2\log^d n.
  \end{align*}
  ...
  Since $\psi$ is surjective, so is $\Psi$, hence there is a string $w$ (of length $h(k) n$) such that $D(w) = 1$.
  Furthermore, this string satisfies $\|w\|_1 \leq h(k)$ since all preimages of $\Psi$ satisfy this inequality.\footnote{%
    Technically, we need to guarantee the satisfying input to $C_x$ is nonzero to make this statement, but it is easy to enforce such a restriction on the witnesses for $Q$.}
  ...
  Then there is a satisfying assignment of length $h(k) \log n$ for $C_x$, namely $\Psi(w)$.
  Therefore, $x$ is in $Q$.
...
\todo{Show this problem is in $\NNC[\poly]$ as well.}
...
\begin{corollary}
  $\pFSAT$ is in $\para \NC^1$ if and only if there is a circuit-computable, nondecreasing, unbounded function $i$ such that $\NC^1 = \NNC^1[i(n) \log n]$.
\end{corollary}
