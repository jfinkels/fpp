...
\todo{This is a miniaturization, incorporate miniaturization into this paper?}
...
\begin{proof}
  ...
  For the size and depth bounds, we need to determine the size and depth of the circuits $U$ and $D_{m, k}$.
  ...
  Hence the overall size of $A_{m, k}$ is of the form $f(k) m^{O(1)}$ and the depth $f(k) + \log^d m$.

  Since we have shown a correct $\para \WNC^d$ circuit family deciding $\pNCSAT$, we conclude that the problem is in $\para \WNC^D$.
\end{proof}
