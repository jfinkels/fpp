...
The weighted evaluation problem is a littly silly, since the Hamming weight of $x$ can be computed trivially, but it is defined to be the ``verification'' problem corresponding to the satisfiability problem above.

\begin{definition}[{$p\dash\textsc{Weighted Circuit Evaluation}$}]
  \mbox{} \\
  \begin{tabular}{r p{9.2cm}}
    \textbf{Instance:} & Boolean circuit $C$, binary string $x$, natural number $k$. \\
    \textbf{Parameter:} & $k$ \\
    \textbf{Question:} & Does the Hamming weight of $x$ equal $k$ and does $C(x) = 1$?
  \end{tabular}
\end{definition}
...
\[
\PW[P] = \cl{p\dash\textsc{Weighted Circuit Evaluation}}^{\leq_m^{\FPP}}.
\]

\begin{theorem}
  $\FPP \subseteq \PW[P] \subseteq \FPT$.
\end{theorem}
\begin{proof}
  For the first inclusion, suppose $(Q, \kappa)$ is an arbitrary parameterized problem in $\FPP$.
  Let $\{C_{n, k}\}$ denote the circuit family that decides it.
  Then the reduction $x \mapsto (C_{|x|, \kappa(x)}, x, \|x\|_1)$ proves membership in $\PW[P]$, where $\|x\|_1$ is the Hamming weight of $x$.
  (This does \emph{not} require $\kappa$ be $\NC$-computable, since we are assuming the reduction is by a nonuniform family of circuits, so the circuit computing the reduction is $R_{n, \kappa(x)}$; if we were using uniform circuit families, then we would require $\kappa$ be $\NC$-computable.)

  For the second inclusion, it suffices to show that the parameterized problem $p\dash\textsc{Weighted Circuit Evaluation}$ is in $\FPT$ (since $\FPP$ reductions compose, $\FPP$ is a subset of $\FPT$, and $\FPT$ is closed under $\FPT$ reductions).
  The underlying decision problem is in $\P$, so the parameterized problem is trivially in $\FPT$.
\end{proof}

\todo{Is $\FPT$ in $\PW$ by the standard reduction from deterministic bounded halting problem to circuit evaluation?}
