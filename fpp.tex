...
The only parts of that formula that involve the variables $x_i$ are checking that another variable is a member of the set $\{x_1, \dotsc, x_k\}$.
In this situation, the variables $x_1, \dotsc, x_k$ can be replaced by a unary relation variable (that is, a set variable) $X$.
This is item two in the lemma below.

\begin{lemma}[{\autocite[Lemma~3.5]{nt16}}]
  Let $n$ and $k$ be positive integers.
  \begin{enumerate}
  \item
    There is a $\Sigma_1$ formula $\textsc{IsMember}_k$ on the free variables $g$ and $x_1, \dotsc, x_k$ such that for each finite group $\mathcal{G}$ of order $n$,
    \[
    \mathcal{G} \models \textsc{IsMember}_k(g, x_1, \dotsc, x_k) \text{ if and only if } g \in \gen{x_1, \dotsc, x_k}.
    \]
  \item
    There is a $\Sigma_1$ formula $\textsc{IsMember}$ on the free variable $g$ and free unary relation symbol $X$ such that for each finite group $\mathcal{G}$ of order $n$,
    \[
    \mathcal{G} \models \textsc{IsMember}(g, X) \text{ if and only if } g \in \gen{X}.
    \]
  \end{enumerate}
\end{lemma}
\todo{Double-check that I understand these formulas correctly.}
%% \begin{proof}
%%   Let $s = \log^2 n$.
...
We can use this formula expressing subgroup membership to give a formula expressing whether a set of group elements is a generating set in the natural way, namely by defining
\[
\textsc{IsGenerating}_k(x_1, \dotsc, x_k) = \forall g \colon \textsc{IsMember}_k(g, x_1, \dotsc, x_k).
\]
Similarly,
\[
\textsc{IsGenerating}(X) = \forall g \colon \textsc{IsMember}(g, X).
\]
...
\begin{lemma}
  ...
  \item
    ...
    \]
  \item
    There is a $\Pi_2$ formula denoted $\textsc{IsGenerating}$ on the free unary relation variables $X$ such that for each finite group $\mathcal{G}$ of order $n$,
    \[
    \mathcal{G} \models \textsc{IsGenerating}(X) \text{ if and only if } \gen{X}= G.
    \]
  \end{enumerate}
\end{lemma}
...
\todo{define first-order reduction, show that it implies an $\NC^1$ reduction.}
Similarly, $\textsc{Group Rank}$ equals $\WD_{\textsc{IsGenerating}}(\textsc{GROUP})$, so $\pgrouprank$ is $\pWD_{\textsc{IsGenerating}}$.
By the definitions of $\WP^d[t]$ and $\ANC^d[t]$, this gives the following complexity upper bounds.

\begin{theorem}
  \mbox{}
  \begin{enumerate}
  \item $\pgrouprank$ is in $\ANC^1[3]$.
  \item $\pgrouprank$ is in $\WP^1[2]$.
  \end{enumerate}
\end{theorem}

\todo{We don't need to include the first item in the above theorem, since the second implies it.}
