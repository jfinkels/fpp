...
%% In \autocite[Lemma~2.4]{nt16}, the authors prove that for each $k$ there is a first-order formula $\textsc{IsMember}_k(g, x_1, \dotsc, x_k)$ such that $\mathcal{G} \models \textsc{IsMember}_k(g, x_1, \dotsc, x_k)$ if and only if $g \in \gen{x_1, \dotsc, x_k}$.
%% The number of alternations is $O(\log n)$, where $n$ is the order of the group.
...
The $\textsc{Group Rank}$ problem then reduces to $\MC(\Pi_2)$ via the reduction $(\mathcal{G}, k) \mapsto (\mathcal{G}, \textsc{IsGenerating}_k)$, where
\[
\textsc{IsGenerating}_k = \textsc{IsGroup} \land \forall g \exists x_1 \dotsb \exists x_k\, (\textsc{IsMember}_k(g, x_1, \dotsc, x_k)).
\]
In the parameterized setting, this proves that $\pgrouprank$ reduces to $\pMC(\Pi_2)$ via a first-order reduction.

\begin{theorem}
  There is an $\FPP^1$ reduction from $\pgrouprank$ to $\pMC(\Pi_2)$.
\end{theorem}

By an analog of \autocite[Corollary~5.16]{fg06} in the setting of fixed-parameter parallelizable reductions, we have $\ANC^1[3] = \cl{\pMC(\Pi_2)}^{\FPP^1}$.
