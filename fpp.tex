...
\begin{definition}\label{def:eventuallyreduction}
  Suppose $d$ is a natural number, $(Q, \kappa)$ is a parameterized problem, and $Q'$ is a decision problem.
  There is \emph{eventually an $\NC^d$ many-one reduction} from $(Q, \kappa)$ to $Q'$ if there is a circuit-computable function $h$ and an $\NC^d$ family of circuits $\{R_n\}_{n \in \mathbb{N}}$ such that for each $x$ of length $n$, if $n \geq h(\kappa(x))$, then $x \in Q$ if and only if $R_n(x) \in Q'$.
\end{definition}
...
\todo{Move these lemmas to an earlier section?}
...
\begin{lemma}
  Suppose $d$ is a natural number, $(Q, \kappa)$ is a parameterized problem, and $Q'$ is a decision problem.
  If $Q'$ is in $\NC^d$ and there is eventually an $\NC^d$ many-one reduction from $(Q, \kappa)$ to $Q'$, then $Q$ is eventually in $\NC^d$.
\end{lemma}
\begin{proof}
  Let $h$ be the function and $\{R_n\}$ the reduction from \autoref{def:eventuallyreduction}.
  Let $\{A_n\}$ be the circuit family deciding $Q'$.
  Then the circuit family consisting of the composition of $A_{n'}$ with $R_n$, where $n'$ is the size of the output of $R_n$, correctly decides $Q$ for inputs $x$ of length at least $h(\kappa(x))$.
  Thus $Q$ is eventually in $\NC^d$.
\end{proof}

Applying the above lemma to \autoref{thm:eventually} yields sufficient conditions for fixed-parameter parallelizability.

\begin{corollary}\label{cor:eventuallyreduction}
  Suppose $d$ is a natural number, $(Q, \kappa)$ is a parameterized problem, and $Q'$ is a decision problem.
  If $Q$ is circuit-decidable, $Q'$ is in $\NC^d$ and there is eventually an $\NC^d$ many-one reduction from $(Q, \kappa)$ to $Q'$, then $(Q, \kappa)$ is in $\FPP^d$.
\end{corollary}
...
\todo{discuss that $\Pi_t \LOGTIME$ is in $\AC^0$, and therefore is strictly weaker than $\NC^1$.}
\todo{Can the \autoref{thm:wppparti} be rephrased as an ``eventually'' reduction?}

\begin{theorem}
  Suppose $d$ is a positive integer, $e$ is a positive integer, and $t$ is a natural number.
  If there is a circuit-computable, nondecreasing, unbounded function $i$ such that $\GC[i(n) \log n, \Pi_t \LOGTIME] \subseteq \NC^d$, then $\FPP^d = \WP^d[t]$.
\end{theorem}
\begin{proof}
  Assume $\GC[i(n) \log n, \Pi_t \LOGTIME] \subseteq \NC^d$.
  Since $\PiCSAT$ is in $\GC[i(n) \log n, \Pi_t \LOGTIME]$ by \autoref{thm:picsatgc}, it is now in $\NC^d$ as well.

  Since $\pPiFSAT$ is complete for $\WP^d[t]$ under $\FPP^d$ many-one reductions, it suffices to show that $\pPiFSAT$ is in $\FPP^d$.
  Furthermore, by \autoref{cor:eventuallyreduction}, it suffices to show that there is eventually an $\NC^d$ many-one reduction from $\pPiFSAT$ to $\PiCSAT$.

  Let $f_i$ be the upper inverse of $i$, as guaranteed by \autoref{lem:upperinverse}.
  ...
  If we can show that $R_n$ is in $\NC^d$, then $f_i$ and $\{R_n\}$ together prove that there is eventually an $\NC^d$ many-one reduction from $\pPiFSAT$ to $\PiCSAT$.
