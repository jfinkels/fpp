...
\begin{definition}
  ...
  A language is \emph{circuit-decidable} if its characteristic function is circuit-computable.
...
\begin{definition}
  TODO Parameterized problem
\end{definition}

\begin{definition}
  TODO NC, NNC, WP[P], FPP, etc.
\end{definition}

\begin{definition}
  Suppose $d$ is a natural number and $(Q, \kappa)$ is a parameterized problem.
  The problem $(Q, \kappa)$ is \emph{eventually in $\NC^d$} if there is a circuit-computable function $h$ and a $\NC^d$ family of circuits $\{C_n\}_{n \in \mathbb{N}}$ such that for each $x$ of length $n$, if $n \geq h(\kappa(x))$, then $x \in Q$ if and only if $C_n(x) = 1$.
\end{definition}
...
\begin{theorem}[Theorem~1.37]\label{thm:eventually}
  Suppose $d$ is a natural number and $(Q, \kappa)$ is a parameterized problem.
...
\begin{theorem}[Proposition~3.7]\label{thm:rmparam}
...
\begin{lemma}[Lemma~3.24]\label{lem:i}
  ...
  \end{itemize}
  Furthermore, these functions are nondecreasing, unbounded, and circuit-computable.
\end{lemma}
\begin{proof}
  Define $i_f$ by
  \[
...
\end{lemma}
\begin{proof}
  The depth-universal circuit of \autocite{ch85} proves membership of this problem in $\WP^d[P]$.
...
\begin{theorem}[Theorem~3.29 part ii]
