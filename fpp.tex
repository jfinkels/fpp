...
These parameterized problems are equivalent under $\para \NC^1$ many-one reductions because the parameterization is computable in $\NC^1$.
...
\section{\texorpdfstring{$\para \NC$}{paraNC} is to \texorpdfstring{$\NC$}{NC} as \texorpdfstring{$\para \WNC$}{WNC} is to \texorpdfstring{$\NNC$}{NNC}}
...
\item The parameterized feedback vertex set problem is in $\para \WL$ \autocite[??]{est15}, which is a subclass of $\para \WNC^2$.
\item The parameterized semigroup rank problem is in $\para \WNL$ \autocite[???]{est15}, which is a subclass of $\para \WNC^2$.
\end{itemize}
...
Let $p \dash k \dash \textsc{FSat}$ denote the weighted Boolean formula satisfiability problem.

\begin{theorem}[{\autocite[Theorem~3.6]{est15}}]\label{thm:parawnc1complete}
  $p \dash k \dash \textsc{FSat}$ is complete for $\para \WNC^1$ under $\para \FO$ many-one reductions.
\end{theorem}

\todo{Give a corollary in the section below that the complete problem is not in $\para \NC$ unless something bad happens.}

Since Boolean formulas are equivalent to Boolean circuits of logarithmic depth \autocite{??}, the above theorem can be rephrased as ``$p \dash \NC^1 \textsc{CSat}$ is complete''.
...
\begin{theorem}[Theorem~3.29, part i]\label{thm:wppparti}
  ...
  If there is a circuit-computable, nondecreasing, unbounded function $i$ such that $\NC^d = \NNC^d[i(n) \log n]$, then $\para \NC^d = \para \WNC^d$.
...
\begin{theorem}\label{thm:wpppartii}
  Suppose $d$ is a positive integer.
  If $\para \NC^d = \para \WNC^d$, then there is a circuit-computable, nondecreasing, unbounded function $i$ such that $\NC^d = \NNC^d[i(n) \log n]$.
\end{theorem}
\begin{proof}
  ...
  \todo{It would be nice to use a complete problem for $\NNC^d[i(n) \log n]$ instead of an arbitrary problem.}
...
This conjecture relaxes \autoref{thm:wnccollapse}.
