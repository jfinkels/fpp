...
Since $\FPP$ reductions compose, $\FPP$ is a subset of $\FPT$, and $\FPT$ is closed under $\FPT$ reductions, we conclude that $\PW[P]$ is a subset of $\FPT$.
...
\todo{Show something bad happens if $\PW[P] = \FPT$.}

\todo{Define $\FPT$-completeness.}

\todo{Show a problem that is known to be in $\FPT$ (for example one of the deterministic Turing machine computation problems maybe) is actually in $\PW[P]$.}

The same issue prevents us from showing that $\FPP \subseteq \PW[P]$.
